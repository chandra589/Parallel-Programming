...
Job* deleteFirst(int index) {
	...
	//printf("In delete first - index is %d and numjbs is %d\n", index, numjbs);
...
void *thread_func(void* arg){
    ...
    while(1){
        ...
        if (item == NULL){
			int queue_num1 = rand()%num_threads;
			int queue_num2 = rand()%num_threads;

			int work1 = allqueues[queue_num1].work;
			int work2 = allqueues[queue_num2].work;
			int queue_num = queue_num1;

			if (work1 >= work2)
				queue_num = queue_num1;
			else
				queue_num = queue_num2;
			
			item = deleteLast(queue_num);
			if (item == NULL){
				count++;
				if (count == numcheck){
        ...
        if (n <= 32)
            ...
            int size = n/2;
            insertFirst(ra, ca, rb, cb, size, index);
            insertFirst(ra, ca, rb, cb + size, size, index);
            insertFirst(ra + size, ca, rb, cb, size, index);
            insertFirst(ra + size, ca, rb, cb + size, size, index);
        }
    }
	...
	while(1){
		...
		if (n <= 32)
			...
			int size = n/2;
			insertFirst(ra, ca + size, rb + size, cb, size, index);
			insertFirst(ra, ca + size, rb + size, cb + size, size, index);
			insertFirst(ra + size, ca + size, rb + size, cb, size, index);
			insertFirst(ra + size, ca + size, rb + size, cb + size, size, index);
		}
	}
